...
import { fetchAllOrdersLastDays, type TrendyolCredentials, type TrendyolOrder } from './trendyol.js';
...
export async function syncStore(
  ...
  const tyCreds: TrendyolCredentials = {
 ...
 */
export async function syncAllStores(token: string, window: BrowserWindow): Promise<void> {
  ...
  for (let i = 0; i < stores.length; i++) {
    ...
    sendProgress(
      window,
      `${store.name} senkronize ediliyor... (${i + 1}/${stores.length})`,
      Math.round((i / stores.length) * 100)
    );
 ...
 */
function normalizeOrder(order: TrendyolOrder, supplierId: string) {
 ...
 */
export function startStoreSync(token: string, window: BrowserWindow) {
