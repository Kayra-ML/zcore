 ...
 */
export function addOrder(order: any, keySeed?: string): Promise<void> {
  return new Promise((resolve, reject) => {
    try {
      ...
      db.run(
        `INSERT OR REPLACE INTO orders (id, storeId, orderDate, encryptedData) VALUES (?, ?, ?, ?)`,
        [id, storeId, orderDate, encryptedData],
        (err) => {
          if (err) reject(err);
          else resolve();
        }
 ...
 */
export function getOrders(days: number = 30, keySeed?: string): Promise<any[]> {
  return new Promise((resolve, reject) => {
