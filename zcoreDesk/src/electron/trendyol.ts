...
export interface TrendyolCredentials {
  ...
  apiKey: string;
  apiSecret: string;
  isStage?: boolean; // true = test ortamı
}
...
export async function fetchOrders(
  ...
  return trendyolRequest<TrendyolOrdersResponse>(creds, path, queryParams);
}
 ...
 */
export async function fetchAllOrdersLastDays(
  creds: TrendyolCredentials,
  days: number = 30,
  ...
  while (windowEnd > now - days * 24 * 60 * 60 * 1000) {
    ...
    if (windowEnd <= now - days * 24 * 60 * 60 * 1000) break;
  }
 ...
 */
export async function validateCredentials(creds: TrendyolCredentials): Promise<{ ok: boolean; error?: string }> {
  try {
